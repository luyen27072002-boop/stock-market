...
def get_symbols() -> List[str]:
    ...
    try:
        data = safe_get_json(FINFO_STOCKS_URL, params={"q": "status:LISTED", "size": 9999})
...
def analyze_full_technical(symbol: str) -> dict:
    ...
    if raw.empty or len(raw) < 80:
        return {"symbol": symbol, "ok": False, "error": "Không đủ dữ liệu."}
    ...
    buy_stop_candidates = [low10, high20 * 0.985 if price > high20 else None, sma20 * 0.97 if sma20 else None, price - atr14 * 1.5 if atr14 else None]
    ...
    technical_stop_candidates = [low10, low20, sma20 * 0.97 if sma20 else None, price - atr14 * 1.5 if atr14 else None]
...
def send_fcm_push(title: str, body: str, data: Optional[dict] = None) -> None:
    ...
    try:
        ...
        if invalid:
            cfg["fcm_tokens"] = [t for t in cfg.get("fcm_tokens", []) if t not in invalid]
            STATE["config"] = cfg
            save_config(cfg)
    except Exception as e:
...
def add_alert(kind: str, symbol: str, title: str, body: str, item: Optional[dict] = None) -> None:
    ...
    send_fcm_push(title, body, {"kind": kind, "symbol": symbol, "url": "/"})


def scan_buy_all(symbols: List[str], min_buy_score: int, max_workers: int) -> List[dict]:
    symbols = list(dict.fromkeys([s.upper().strip() for s in symbols if s.strip()]))[:800]
    results = []

    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        future_map = {pool.submit(analyze_full_technical, s): s for s in symbols}
    ...
    results = sorted(results, key=lambda x: (x.get("buy_score", 0), x.get("vol_ratio", 0), x.get("day_pct", 0)), reverse=True)
    ...
    return [x for x in results if x.get("buy_score", 0) >= 60][:100]
...
def scan_positions(positions: List[dict], min_sell_score: int, max_workers: int) -> List[dict]:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        future_map = {pool.submit(work, p): p for p in positions if str(p.get("symbol", "")).strip()}
        for fut in as_completed(future_map):
            try:
                item = fut.result()
                if item.get("ok"):
    ...
    return sorted(results, key=lambda x: (x.get("sell_score", 0), -x.get("day_pct", 0)), reverse=True)
...
def api_set_config(payload: dict = Body(...)):
    cfg = STATE["config"]
    allowed = [
        "auto_scan_enabled", "scan_interval_sec", "min_buy_score", "min_sell_score",
        "scan_symbols", "max_workers", "alert_cooldown_sec", "positions",
        "firebase_enabled", "firebase_service_account_path", "firebase_vapid_key",
        "firebase_web_config", "fcm_tokens",
    ]
    for k in allowed:
        if k in payload:
            cfg[k] = payload[k]
    ...
    if isinstance(cfg.get("scan_symbols"), str):
        cfg["scan_symbols"] = [x.strip().upper() for x in cfg["scan_symbols"].replace("\n", ",").split(",") if x.strip()]
    cfg["scan_symbols"] = list(dict.fromkeys([str(s).upper().strip() for s in cfg.get("scan_symbols", []) if str(s).strip()]))
...
def api_alerts(since_id: int = Query(default=0)):
    alerts = [a for a in STATE["alerts"] if int(a.get("id", 0)) > since_id]
    return {
        "count": len(alerts),
        "alerts": alerts,
        "latest_id": max([int(a.get("id", 0)) for a in STATE["alerts"]] or [since_id]),
    }
...
def api_scan_now():
    ...
    STATE["last_scan_at"] = now_str()
    STATE["scan_count"] += 1
    return {"ok": True, "buy_results": buy_results, "position_results": pos_results}
...
def api_fcm_register(payload: dict = Body(...)):
    ...
    cfg = STATE["config"]
    tokens = list(dict.fromkeys(cfg.get("fcm_tokens", []) + [token]))
    cfg["fcm_tokens"] = tokens[-2000:]
    STATE["config"] = cfg
    save_config(cfg)
    return {"ok": True, "token_count": len(cfg["fcm_tokens"])}


@app.post("/api/fcm/test")
def api_fcm_test():
    send_fcm_push("Test Stock Alert", "Nếu thấy thông báo này thì Firebase Push đã hoạt động.", {"kind": "TEST", "url": "/"})
    return {"ok": True}
